...
/* ============================================================
   THE VALE'S GENERATED SKY

   vale-sky.css is written by tools/vale.mjs and consumed by
   vale.css. Two files, one produced from the other, and this repo
   ...
   ============================================================ */
import { readFileSync as readVale } from 'node:fs'

const valeCss = readVale(new URL('../public/css/vale.css', import.meta.url), 'utf8')
const skyCss = readVale(new URL('../public/css/vale-sky.css', import.meta.url), 'utf8')
const valeTool = readVale(new URL('../tools/vale.mjs', import.meta.url), 'utf8')

test('every --vale-* token vale.css uses is one vale-sky.css defines', () => {
  const used = new Set([...valeCss.matchAll(/var\((--vale-[a-z-]+)/g)].map(m => m[1]))
  const defined = new Set([...skyCss.matchAll(/(--vale-[a-z-]+)\s*:/g)].map(m => m[1]))
  assert.ok(used.size > 3, 'expected vale.css to consume the generated tokens, found ' + used.size)
  for (const token of used) {
    assert.ok(defined.has(token),
      token + ' is used in vale.css but never defined in vale-sky.css. ' +
...
test('the generator still emits every token, so a regen cannot drop one', () => {
  /* The other direction, and the one a rename actually breaks: the
     ...
     tool, the next `node tools/vale.mjs` deletes it and the page
     loses a stop. Asserting on the tool's source rather than running
     it keeps this test from writing files. */
  for (const token of [...skyCss.matchAll(/(--vale-[a-z-]+)\s*:/g)].map(m => m[1])) {
    assert.ok(valeTool.includes(token),
      token + ' is in the committed vale-sky.css but tools/vale.mjs no longer emits it. ' +
      'A regen would silently drop it.')
  }
})

test('vale-sky.css says it is generated', () => {
  ...
  assert.match(skyCss.slice(0, 160), /GENERATED by tools\/vale\.mjs/)
})
